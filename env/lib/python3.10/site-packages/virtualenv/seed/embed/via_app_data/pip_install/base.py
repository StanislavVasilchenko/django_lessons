...
class PipInstall(metaclass=ABCMeta):
    ...
    def install(self, version_info):
        ...
        for name, module in self._console_scripts.items():
            consoles.update(self._create_console_entry_point(
    ...
    def build_image(self):
        ...
        logging.debug("build install image for %s to %s",
                      self._wheel.name, self._image_dir)
    ...
    def _uninstall_dist(dist):
        ...
        logging.debug("uninstall existing distribution %s from %s",
                      dist.stem, dist_base)
        ...
        for name in (
            (i.split(",")[0] for i in record.read_text(
