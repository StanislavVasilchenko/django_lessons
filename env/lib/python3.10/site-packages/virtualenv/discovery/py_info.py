...
class PythonInfo:
    ...
    def __init__(self) -> None:  # noqa: PLR0915
        ...
        self.real_prefix = abs_path(
            getattr(sys, "real_prefix", None))  # old virtualenv
        ...
        self.sysconfig_vars = {i: sysconfig.get_config_var(
            i or "") for i in config_var_keys}

        confs = {
            k: (self.system_prefix if v is not None and v.startswith(
                self.prefix) else v)
    ...
    def install_path(self, key):
        ...
        if result is None:  # use sysconfig if sysconfig_scheme is set or distutils is unavailable
            ...
            config_var = {k: "" if v in prefixes else v for k,
                          v in self.sysconfig_vars.items()}
            result = self.sysconfig_path(
                key, config_var=config_var).lstrip(os.sep)
        return result
    ...
    def _to_dict(self):
        data = {var: (getattr(self, var) if var not in (
            "_creators",) else None) for var in vars(self)}
    ...
    def from_exe(  # noqa: PLR0913
        ...
        proposed = from_exe(cls, app_data, exe, env=env,
                            raise_on_error=raise_on_error, ignore_cache=ignore_cache)
    ...
    def _resolve_to_system(cls, app_data, target):
        ...
        while target.system_executable is None:
            ...
            if prefix in prefixes:
                ...
                logging.error("%d: prefix=%s, info=%r",
                              len(prefixes) + 1, prefix, target)
    ...
    def discover_exe(self, app_data, prefix, exact=True, env=None):  # noqa: FBT002
        ...
        if key in self._cache_exe_discovery and prefix:
            logging.debug("discover exe from cache %s - exact %s: %r",
                          prefix, exact, self._cache_exe_discovery[key])
        ...
        if exact is False and discovered:
            ...
            self._cache_exe_discovery[key] = info
            logging.debug(
                "no exact match found, chosen most similar of %s within base folders %s", info, folders)
            return info
    ...
    def _check_exe(self, app_data, folder, name, exact, discovered, env):  # noqa: PLR0913
        ...
        for item in ["implementation", "architecture", "version_info"]:
            ...
            if found != searched:
                ...
                logging.debug("refused interpreter %s because %s differs %s != %s",
                              executable, item, found, searched)
                if exact is False:
