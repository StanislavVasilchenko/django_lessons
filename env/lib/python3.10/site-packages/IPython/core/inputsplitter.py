...
dedent_re = re.compile(
    "|".join(
        [
            # raise statement (+ space + other stuff, maybe)
            r"^\s+raise(\s.*)?$",
            r"^\s+raise\([^\)]*\).*$",  # wacky raise with immediate open paren
            # normal return (+ space + other stuff, maybe)
            r"^\s+return(\s.*)?$",
            # wacky return with immediate open paren
            r"^\s+return\([^\)]*\).*$",
            r"^\s+pass\s*$",  # pass (optionally followed by trailing spaces)
            r"^\s+break\s*$",  # break (optionally followed by trailing spaces)
            # continue (optionally followed by trailing spaces)
            r"^\s+continue\s*$",
        ]
    )
)
