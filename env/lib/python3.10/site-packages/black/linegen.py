...
def _first_right_hand_split(
    ...
    if Preview.hug_parens_with_braces_and_square_brackets in line.mode:
        is_unpacking = 1 if body_leaves[0].type in [
            token.STAR, token.DOUBLESTAR] else 0
        if (
        ...
        try:
            ...
            if not (
                ...
                and is_line_short_enough(
                    rhs.head, mode=replace(
                        mode, line_length=mode.line_length - 1)
                )
            ...
            if not (
                can_be_split(rhs.body) or is_line_short_enough(
                    rhs.body, mode=mode)
            ):
                raise CannotSplit(
