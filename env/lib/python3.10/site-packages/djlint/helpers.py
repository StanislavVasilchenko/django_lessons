...
def is_ignored_block_opening(config: Config, item: str) -> bool:
    ...
    return bool(
        re.search(
            re.compile(config.ignored_block_opening,
...
def is_script_style_block_opening(config: Config, item: str) -> bool:
    ...
    return bool(
        re.search(
            re.compile(config.script_style_opening,
                       re.IGNORECASE | re.VERBOSE),
            item[last_index:],
        )
    )
...
def is_ignored_block_closing(config: Config, item: str) -> bool:
    ...
    inline = list(
        re.finditer(
            re.compile(config.ignored_inline_blocks,
                       flags=re.IGNORECASE | re.VERBOSE),
            item,
        )
    )
    ...
    return re.search(
        re.compile(config.ignored_block_closing,
                   flags=re.IGNORECASE | re.VERBOSE),
        item[last_index:],
    )
...
def is_script_style_block_closing(config: Config, item: str) -> bool:
    ...
    inline = list(
        re.finditer(
            re.compile(config.script_style_inline,
                       flags=re.IGNORECASE | re.VERBOSE),
            item,
        )
    )
    ...
    return re.search(
        re.compile(config.script_style_closing,
                   flags=re.IGNORECASE | re.VERBOSE),
        item[last_index:],
    )
...
def inside_ignored_rule(config: Config, html: str, match: re.Match, rule: str) -> bool:
    ...
    for rule_regex in config.ignored_rules:
        for ignored_match in list(
            ...
            if (
                rule in list(
                    set(re.split(r"\s|,", ignored_match.group(1).strip())))
                and (
                    ignored_match.start(0) <= match.start()
                    and match.start() <= ignored_match.end()
                )
