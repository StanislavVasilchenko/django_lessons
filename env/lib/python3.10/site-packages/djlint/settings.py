...
def load_project_settings(src: Path, config: Optional[str]) -> Dict:
    ...
    if djlintrc_file:
        ...
        # pylint: disable=broad-except
            ...
            logger.error("%sFailed to load .djlintrc file. %s",
                         Fore.RED, error)
...
def build_ignore_blocks(ignore_blocks: Union[str, None]) -> Optional[str]:
    ...
    if ignore_blocks:
        ...
        close_tags = [
            "end" + x.strip() + r"\b" for x in ignore_blocks.split(",")]
...
class Config:
    ...
    def __init__(
        ...
        self.custom_html: str = str(
            build_custom_html(
                custom_html or djlint_settings.get("custom_html")) or ""
        )
        ...
        self.js_config = (
            {"indent_size": indent_js} if indent_js else djlint_settings.get(
                "js")
        ) or {}

        self.css_config = (
            {"indent_size": indent_css} if indent_css else djlint_settings.get(
                "css")
        ) or {}
        ...
        self.indent_html_tags: str = "|".join(
            html_tag_names) + self.custom_html
        ...
        try:
            self.max_attribute_length = max_attribute_length or int(
                djlint_settings.get("max_attribute_length",
