...
def print_output(
    ...
    if config.statistics and config.lint:
        build_stats_output([x.get("lint_message")
                           for x in file_errors], config)
...
def build_output(error: dict, config: Config) -> int:
    ...
    errors = sorted(
        list(error.values())[0], key=lambda x: tuple(
            map(int, x["line"].split(":")))
    )
    ...
    filename = build_relative_path(
        list(error.keys())[0], config.project_root.resolve())
...
def build_check_output(errors: dict, config: Config) -> int:
    ...
    if config.quiet is False and len(list(errors.values())[0]) > 0:
        echo(
            ...
            + Style.BRIGHT
            + "\n"
            + build_relative_path(list(errors.keys())
                                  [0], config.project_root.resolve())
            + "\n"
            + Style.DIM
            + "".join(["─" for x in range(1, width)])
            + Style.RESET_ALL
